# Official scorer, vendored so our numbers are comparable to published ones.
#
# `extract_mcq_answer` below is a VERBATIM copy of
#     lmms-eval/lmms_eval/tasks/_task_utils/mcq_extract.py
# (EvolvingLMMs-Lab/lmms-eval, main @ 2026-08-13), which upstream's `lvbench.yaml`
# uses via `lvbench_process_results` -> `extract_characters_regex`. That is the
# scorer behind the Qwen2.5-VL-7B LVBench numbers in the LongVT paper (Table 2:
# 30.7 sparse / 40.9 dense), so reproducing those numbers requires reproducing
# this extractor exactly. Do not "improve" it.
#
# NOT vendored: LongVT's own custom_rewards/lmms_lab_recipe.py::compute_score.
# It falls back to an LLM judge (they used Qwen3-235B-A22B) which we cannot run,
# so importing it would add an unreproducible dependency. Our arms are scored with
# `data.extract_answer` (agentic, <answer>-tag aware) AND with the function below;
# both columns are written so the scorer's own contribution stays visible.
#
# --- begin verbatim copy ---
