"""Base-Qwen probe for the baseline plan: does BASE Qwen2.5-VL-7B-Instruct (the
untrained model LongVT-RFT was fine-tuned from) drive the same LongVT tool loop
with compress_video/crop_video registered? This is baseline arm 2 of the plan:
    video -> compression tool -> compressed evidence -> Qwen -> answer
Mirrors compress_smoke.py exactly, but targets the base-model server (port 8001).
NOTE: compress_video is still the PIXEL-SPACE coverage stand-in, not real FlashVID.
"""
